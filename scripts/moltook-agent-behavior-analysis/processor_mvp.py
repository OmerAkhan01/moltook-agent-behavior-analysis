...
RAW_CSV_PATH = ROOT_DIR / "data" / "raw" / "moltbook_raw.csv"
...
dil_analizi.to_csv(PROCESSED_DIR / "dil_analizi.csv", index=False)
...
cluster_df.sample(n=min(5000, len(cluster_df))).to_csv(PROCESSED_DIR / "ajan_kumeleri.csv", index=False)
...
try:
    ...
    forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].to_csv(PROCESSED_DIR / "prophet_tahmin.csv", index=False)
