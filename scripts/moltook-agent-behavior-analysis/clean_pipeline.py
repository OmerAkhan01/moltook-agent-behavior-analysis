...
def clean_and_integrate_pipeline(cleaned_path, raw_path, output_path):
    ...
    try:
        # 1. Veri Okuma
        df_cleaned = pd.read_csv(_resolve_path(cleaned_path))
        df_raw = pd.read_csv(_resolve_path(raw_path))
        ...
        # 7. Çıktı Üretme (H7)
        df_final.to_csv(_resolve_path(output_path), index=False)
        
        logging.info(f"Sonuç: İşlem tamamlandı. {len(df_final)} satır '{_resolve_path(output_path)}' dosyasına kaydedildi.")
